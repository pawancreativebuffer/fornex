...
export default function Results() {
    const listData = [
        "Teachers report feeling significantly more confident about student progress documentation - the guesswork has been removed entirely",
        "Admin staff describe the student record system as faster and more reliable than any manual method previously used",
        "The parent messaging feature is consistently rated the highest - value interaction - eliminating the most communication - prone step in the workflow",
        "Having formal progress reports generated automatically is described as the single biggest improvement to the documentation process",
        "The color - coded assessment result system is immediately intuitive - no training required to interpret and act on a student's status with confidence"
    ]

    const potential = [
        {
            title: "For Special Education Schools",
            desc: "Fewer documentation gaps, faster student throughput, a standardized management process that no longer depends on individual staff knowledge",
            desc_p1: "Consistent outcomes regardless of which teacher is on shift",
            color: "#60C6B1",
            icon: GraduationCap,
            num: "01"
        },
        {
            title: "For Families",
            desc: "Real-time access to their child's daily progress, previously unavailable through any structured channel",
            desc_p1: "Parents no longer wait days for updates - they see everything as it happens",
            color: "#90c7e5",
            icon: Heart,
            num: "02"
        },
        {
            title: "For Educational System",
            desc: "Reduced administrative waste, stronger audit trails, and a consistent compliance protocol that scales across every unit",
            desc_p1: "Every student, every clinician, every shift - standardized",
            color: "#ff9900",
            icon: ShieldCheck,
            num: "03"
        }
            ...
            <div className="max-w-[1400px] mx-auto px-4">
                <div className="flex flex-col justify-center text-center gap-5">
                    ...
                    <h1 className="text-shadow-lg/20 font-[700] text-4xl lg:text-6xl text-[#fff] leading-[1.1] max-w-4xl m-auto">
                        Early Access Validated <span className="text-[#60C6B1]">Real Educators. Real Impact.</span>
                    </h1>
                    <p className="text-white/70 text-base leading-relaxed max-w-5xl m-auto">
                        SuperSchool is currently in active deployment with its initial user base providing structured feedback. Qualitative outcomes from early use have validated the platform's design against the most critical pain points in special education management workflows.
                    </p>
                </div>
                    ...
                    {potential.map((card, index) => (
                        <div key={index} className="relative flex flex-col items-center group">
                            ...
                            <div className="w-full h-full bg-white/[0.03] border-2 rounded-3xl pt-12 pb-24 px-8 text-center transition-all duration-500 group-hover:bg-white/[0.06] group-hover:-translate-y-3" style={{ borderColor: `${card.color}40` }}>
                                ...
                                <p className="text-white/70 text-sm mt-5 leading-relaxed group-hover:text-white transition-colors duration-300">{card.desc_p1}</p>
                            </div>
