...
const services = [
    ...
    {
        id: 3,
        title: "Healthcare Website & Mobile App Development",
            ...
            <div className="w-full mx-auto px-2">
                <div className="flex flex-col md:flex-row w-full h-[700px] md:h-[500px] gap-2">
                    {services.map((service, index) => (
                        <div
                            key={service.id}
                            className={`relative overflow-hidden rounded-[2rem] transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] cursor-pointer group ${activeIndex === index ? 'flex-[12] md:flex-[5]' : 'flex-[1.5] md:flex-[1]'
                                }`}
                        ...
                        >
                            ...
                            <div className="absolute inset-0 p-6 md:p-10 flex flex-col justify-end">
                                ...
                                <div className={`transition-all duration-700 delay-100 ${activeIndex === index ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'
                                    ...
                                    <span className="inline-block px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-xs font-semibold text-white mb-4 tracking-wider uppercase">
                                        Service 0{index + 1}
                                    </span>
                                ...
                                <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-500 ${activeIndex === index ? 'opacity-0' : 'opacity-100'
                                    ...
                                    <div className="relative h-full w-full flex items-center justify-center">
                                        <h3 className="text-white text-lg md:text-xl font-bold whitespace-nowrap -rotate-90 origin-center tracking-wide uppercase opacity-80 group-hover:opacity-100 transition-all duration-300">
                                            {service.title}
                                        </h3>
                                    </div>
                                </div>
                                ...
                                <div className={`absolute top-10 left-10 transition-opacity duration-500 ${activeIndex === index ? 'opacity-0' : 'opacity-100'
                                    }`}>
                                    <span className="text-white/40 font-mono text-lg font-light tracking-tighter">0{index + 1}</span>
                                </div>
                            </div>
                        </div>
                    ))}
